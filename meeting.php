...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM event WHERE DATE(last_updated) = CURDATE() AND id_account = :id_account AND meet_1 IS NOT NULL");
    ...
    if ($count > 0) {
        header('Location: meeting_done.php');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    if (isset($_POST['project']) && ($_POST['project'] === 'มา' || $_POST['project'] === 'ไม่มา')) {
        ...
        try {
            $updateStmt = $pdo->prepare("INSERT INTO event (meet_1, id_account, last_updated) VALUES (:update_value, :id_account, NOW())");
                ...
                <div class="container my-5 py-5 px-lg-5">
                    <div class="row g-5 py-5">
                        <div class="col-12 text-center">
                            <h1 class="text-white animated zoomIn">ร่วมประชุม</h1>
                        </div>
                    </div>
                </div>
            </div>
