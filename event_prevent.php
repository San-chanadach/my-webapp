...
try {
    ...
    $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM event WHERE DATE(last_updated) = CURDATE() AND id_account = :id_account AND prevent IS NOT NULL");
    ...
    if ($count > 0) {
        header('Location: event_prevent_done.php');
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    if (isset($_POST['project']) && ($_POST['project'] === 'มา' || $_POST['project'] === 'ไม่มา')) {
        ...
        try {
            $updateStmt = $pdo->prepare("INSERT INTO event (prevent, event_note, id_account, last_updated) VALUES (:update_value, :event_note, :id_account, NOW())");
                    ...
                    <div class="container my-5 py-5 px-lg-5">
                        <div class="row g-5 py-5">
                            <div class="col-12 text-center">
                                <h1 class="text-white animated zoomIn">งานป้องกัน</h1>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <form method="POST">
                                ...
                                <div class="wrap-input100 validate-input">
                                    <select id="sub_event_dropdown" class="input100 text-center" name="sub_event_id" required>
                                        ...
                                        <?php
                                        require_once 'login/connect.php';
                                        $sql = "SELECT id_sub_event, name FROM sub_event WHERE type = 'prevent'";
